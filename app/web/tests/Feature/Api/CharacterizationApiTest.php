...
it('exposes the optional activity questions block in the options payload', function () {
    $this->actingAs($this->user)
        ->getJson('/api/characterization/options')
        ->assertOk()
        ->assertJsonPath('data.levels.activity_questions.fields_prefix', 'form_data.activity_questions')
        ->assertJsonPath('data.levels.activity_questions.values.not_sure', 'No estoy seguro')
        ->assertJsonPath(
            'data.levels.activity_questions.fields.water_use',
            '¿Alguna actividad propia usa o vierte agua de forma relevante (refrigeración, limpieza industrial, procesado de alimentos, riego, teñido…)?'
        )
        ->assertJsonCount(9, 'data.levels.activity_questions.fields');
});

it('stores optional activity question answers with the draft', function () {
    Bus::fake();
    ...
    $this->actingAs($this->user)
        ->putJson('/api/characterization', validApiCharacterizationPayload($topicId, [
            ...
            'form_data' => [
                'activity_questions' => [
                    'physical_operations' => 'yes',
                    'water_use' => 'no',
                    'international_footprint' => 'not_sure',
                ],
            ],
        ]))
        ->assertOk()
        ->assertJsonPath('data.form_data.activity_questions.physical_operations', 'yes')
        ->assertJsonPath('data.form_data.activity_questions.water_use', 'no')
        ->assertJsonPath('data.form_data.activity_questions.international_footprint', 'not_sure');

    $characterization = Characterization::where('user_id', $this->user->id)->first();

    expect($characterization->form_data['activity_questions']['physical_operations'])->toBe('yes');
});

it('rejects activity question answers outside the yes/no/not_sure values', function () {
    Bus::fake();
    ...
    $this->actingAs($this->user)
        ...
        ->assertStatus(422)
        ->assertJsonValidationErrors(['form_data.activity_questions.water_use']);
});

it('submits successfully without any activity question answers', function () {
    Bus::fake();
