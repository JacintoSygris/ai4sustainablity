...
it('serves protected APIs through Laravel web session middleware for browser clients', function () {
    $middleware = Route::getRoutes()->getByName('api.characterization.show')?->gatherMiddleware() ?? [];

    expect($middleware)
        ->toContain('web')
        ->toContain('auth')
        ->toContain('verified.required');
});
