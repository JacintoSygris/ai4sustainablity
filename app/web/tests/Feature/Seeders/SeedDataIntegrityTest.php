...
it('keeps Python ESRS key inventory covered by the AR16 runtime mapping', function () {
    $inventoryPath = base_path('data/python-esrs-key-inventory.csv');

    expect(is_file($inventoryPath))->toBeTrue();
    expect(filesize($inventoryPath))->toBe(5945);
    expect(hash_file('sha256', $inventoryPath))->toBe('45f016c8110017529496ea50b17f556f75585df9cfab4f7cf3b882fbc7eb92a9');

    $inventoryKeys = collect(readCsvRows($inventoryPath))
    ...
    expect($inventoryKeys)->toHaveCount(96);
...
it('keeps the new-format 732 mapping inventory aligned with AR16 topics', function () {
    $mapping = json_decode(
