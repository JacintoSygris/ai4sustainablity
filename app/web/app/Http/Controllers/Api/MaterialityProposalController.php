...
class MaterialityProposalController extends Controller
{
    ...
    private function proposalState(Characterization $characterization): array
    {
        ...
        // ADD-only extension (P6 document extraction contract v0): when the
        // feature flag is ON, append the document_evidence block. Existing
        // fields and the base topic list stay byte-identical either way.
        if (config('services.p6_document_upload.enabled')) {
            $state['document_evidence'] = $this->documentEvidencePresenter->present($characterization);
        }
