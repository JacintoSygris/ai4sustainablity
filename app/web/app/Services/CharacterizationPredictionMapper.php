...
class CharacterizationPredictionMapper
{
    ...
    /**
     * Resolves a Python ESRS prediction key (extraction topic_key) to the AR16
     * ESRS code, only through the approved mapping inventory. Unresolved or
     * unapproved keys return null (standard-level context; never fabricated).
     */
    public function esrsCodeForPredictionKey(?string $key): ?string
    {
        ...
        if ($this->usesNewFormatInventory()) {
            $row = collect(Arr::get($mapping, 'keys', []))
                ->first(fn (array $row) => ($row['python_esrs_key'] ?? null) === $key
                    && ($row['status'] ?? null) === 'approved'
                    && is_array($row['ar16_topic_ids'] ?? null)
                    && $row['ar16_topic_ids'] !== []);
        } else {
            $row = collect(Arr::get($mapping, 'candidate_topics', []))
                ->first(fn (array $row) => ($row['mapping_status'] ?? null) === 'approved'
                    && in_array($key, $row['python_esrs_keys'] ?? [], true));
        }
        ...
        $webEsrs = $row['web_esrs'] ?? null;

        return is_string($webEsrs) && filled($webEsrs) ? $webEsrs : null;
    }
