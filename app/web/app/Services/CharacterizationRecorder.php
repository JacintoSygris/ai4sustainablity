...
class CharacterizationRecorder
{
    ...
    private function saveOnce(User $user, array $payload): Characterization
    {
        ...
        $activityQuestionsPayload = Arr::get($payload, 'form_data.activity_questions', []);
        if (is_array($activityQuestionsPayload) && ! empty($activityQuestionsPayload)) {
            $existingAnswers = Arr::get($formData, 'activity_questions', []);
            $formData['activity_questions'] = array_merge(
                is_array($existingAnswers) ? $existingAnswers : [],
                array_filter(
                    $activityQuestionsPayload,
                    fn ($value, $key) => is_string($value) && array_key_exists($key, CharacterizationOptions::activityQuestions()),
